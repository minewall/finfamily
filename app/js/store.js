...
const Store = (function () {
  ...
  // ─── Parser determinístico de extratos bancários (CSV/OFX) ──────
  // Recebe texto cru + tipo, retorna estrutura padronizada de transações
  // pra revisão. Não persiste nada. Sem categorização final — só keyword
  // hint pro Coach refinar via tool use depois.
  //
  // Fixtures de referência (uso mental, não rodam):
  //
  //   OFX:
  //     <STMTTRN><TRNTYPE>DEBIT</TRNTYPE><DTPOSTED>20260115120000</DTPOSTED>
  //     <TRNAMT>-45.90</TRNAMT><MEMO>UBER TRIP</MEMO></STMTTRN>
  //
  //   Itaú CSV (cp1252/utf-8, ;):
  //     "Data";"Histórico";"Valor"
  //     "15/01/2026";"UBER *TRIP";"-45,90"
  //     "16/01/2026";"SALARIO";"5.200,00"
  //
  //   Nubank CSV (utf-8, ,):
  //     Data,Categoria,Título,Valor
  //     2026-01-15,Transporte,Uber,-45.90
  //     2026-01-20,Alimentação,iFood Pedido,-78.50
  //
  //   Genérico:
  //     date|description|amount
  //     2026-01-15|Boleto agua|-120.00
  //
  ...
  function _normalizarValor(str) {
    ...
    if (lastDot >= 0 && lastComma >= 0) {
      if (lastComma > lastDot) {
        // vírgula é decimal → remove pontos (milhares), troca vírgula por ponto
  ...
  const _KEYWORD_CATS = [
    { re: /\b(uber|99|99pop|99\s*taxi|taxi|cabify|metr[oô]|metr[oô]\s*sp|cptm|bilhete[\s-]*[uú]nico|pedagio|shell\s*box|posto|combust[ií]vel|gasolina|[aá]lcool|estacion|zona\s*azul)\b/i, cat: 'transporte' },
    ...
    { re: /\b(shopping|mercado|extra|carrefour|p[aã]o\s*de\s*a[cç][uú]car|assa[ií]|atacad[aã]o|sams|makro|hortifruti|sacol[aã]o|mercearia)\b/i, cat: 'mercado' },
    ...
    { re: /\b(vivo|claro|tim|oi|net\s*claro|internet|telefonia|celular\s*pos)\b/i, cat: 'telecom' },
    ...
    { re: /\b(saque|saq\.|atm|caixa\s*eletr)\b/i, cat: 'saque' },
    { re: /\b(tarifa|anuidade|iof|juros|multa|encargo)\b/i, cat: 'tarifas' },
  ];

  function _categorizarPorKeyword(descricao) {
  ...
  function _detectSeparator(sample) {
  ...
  function _parseOFX(text) {
    ...
    const errors = [];
    const txs = [];
    // Pega só seção de transações pra evitar STMTTRN aninhados estranhos
    const body = _stripBOM(String(text || ''));
    if (!/<OFX|OFXHEADER/i.test(body)) {
      errors.push('Arquivo não parece OFX (sem header <OFX> ou OFXHEADER)');
    }
    const blocks = body.match(/<STMTTRN>[\s\S]*?<\/STMTTRN>/gi) || [];
    if (!blocks.length) {
      // Algumas variantes não fecham as tags — tenta heurística com FITID
  ...
  function _parseCsvCommon(text, opts) {
    // opts: { formato, sep, dateCol, descCol, valorCol, hasHeader, headerMap? }
    const warnings = [];
    const errors = [];
    const txs = [];
    const lines = _splitLines(text).filter(l => l.trim().length > 0);
    let startIdx = opts.hasHeader ? 1 : 0;
    ...
    if (dateCol == null || descCol == null || valorCol == null || dateCol < 0 || descCol < 0 || valorCol < 0) {
    ...
    for (let i = startIdx; i < lines.length; i++) {
      ...
      if (!dt) {
        warnings.push('Linha ' + (i + 1) + ' ignorada (data inválida: "' + dataStr + '")');
        continue;
      }
      if (!Number.isFinite(valor)) {
        warnings.push('Linha ' + (i + 1) + ' ignorada (valor inválido: "' + valorStr + '")');
        continue;
      }
      idx++;
      txs.push({
  ...
  function _parseNubankCsv(text) {
    return _parseCsvCommon(text, {
      formato: 'nubank-csv',
      sep: ',',
      dateCol: 0, descCol: 2, valorCol: 3,
      hasHeader: true,
  ...
  function _parseCsvGenerico(text) {
    const lines = _splitLines(text).filter(l => l.trim().length > 0);
    if (!lines.length) {
      return { formato: 'csv-generico', transacoes: [], totalLinhas: 0, warnings: [], errors: ['Arquivo vazio'] };
    }
  ...
  function parseExtrato({ tipo, conteudo, opcoes } = {}) {
    const out = {
      ok: false,
      formato: null,
      totalLinhas: 0,
    ...
    let fmt = (tipo && tipo !== 'auto') ? tipo : _detectFormat(conteudo);
    let parsed;
    try {
      ...
      out.errors.push('Falha ao parsear: ' + (e && e.message ? e.message : String(e)));
      out.formato = fmt;
      return out;
    }
    ...
    parsed.transacoes.forEach((tx, i) => {
      ...
      if (seen.has(key)) {
        out.warnings.push('possível duplicata: linha ' + (seen.get(key) + 1) + ' e ' + (i + 1) + ' idênticas');
      } else {
        seen.set(key, i);
      }
